...
for f in files:
    ...
    for l in logcat_list:
        only_logcat_time.append(l['SyscTime'])
    for e in event_list:
        only_event_time.append(e['SyscTime'])
    event_logcat = []
    for e in only_event_time:
        val = []
        for l in only_logcat_time:
            val.append(abs(l - e))
        event_logcat.append(min(val))
    print(max(event_logcat))
