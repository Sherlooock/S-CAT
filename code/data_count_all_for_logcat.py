...
for l_root in logcat_files:
    if l_root in event_files:
        logcat_context = open(logcat_files[l_root]).readlines()
        logcat_all_list=[]
        r+=1;
        ...
        event_all_list = []
        event_all = open(event_files[l_root]).readlines()
        ...
        for l in logcat_all_list:
            if l['priority'] in logcat_filter:
                flag = 0
                count+=1
                # for e in event_all_list:
                #     if l['SyscTime']>=e['SyscTime'] and l['SyscTime'] - e['SyscTime']<=1000:
                #         print e
                #         flag = 1
                # if(flag==1):
                #     print l
                #     print ''
        if len(re.findall('com.taobao.ishopping',logcat_files[l_root]))!=0:
            print logcat_files[l_root],count
print r