...
for f in files:
    ...
    logcat_little_sequence_by_time = []
    for i in range(len(logcat_list)-1):
        time = logcat_list[i+1]['SyscTime']-logcat_list[i]['SyscTime']
        if time < 2000:
            logcat_little_sequence_by_time.append(logcat_list[i])
        else:
            logcat_little_sequence_by_time.append(logcat_list[i])
            logcat_sequence_by_time.append(logcat_little_sequence_by_time)
            logcat_little_sequence_by_time = []
    logcat_little_sequence_by_time.append(len(logcat_list)-1)
    logcat_sequence_by_time.append(logcat_little_sequence_by_time)
print(len(logcat_sequence_by_time))
logcat_sequence = ''
for logcat_sequence_by_time_list in logcat_sequence_by_time:
    for e in logcat_sequence_by_time_list:
        try:
